...
#define SLIMVALUE_VERSION_FULL "@SLIMVALUE_VERSION_FULL@"
...
/* 				else {
			...
			std::optional<slim_coordinates> try_coordinates() const {
				if (is<slim_coordinates>()) return get<slim_coordinates>();
				return std::nullopt;
			}
...
struct SlimMultiMap {
	...
	struct SlimValue {
		...
		public:
			...
			// BUG FIX: was calling std::any_cast on an AnyValue — now correctly delegates
			template <typename _T>
			const _T& get() const {
				return __value.get<_T>();
			}
			...
			bool get_bool() const                             { return __value.get_bool(); }
			int get_int() const                               { return __value.get_int(); }
			long get_long() const                             { return __value.get_long(); }
			long long get_long_long() const                   { return __value.get_long_long(); }
			unsigned get_unsigned() const                     { return __value.get_unsigned(); }
			unsigned long get_unsigned_long() const           { return __value.get_unsigned_long(); }
			unsigned long long get_unsigned_long_long() const { return __value.get_unsigned_long_long(); }
			float get_float() const                           { return __value.get_float(); }
			double get_double() const                         { return __value.get_double(); }
			long double get_long_double() const               { return __value.get_long_double(); }
			char get_char() const                             { return __value.get_char(); }
			const std::string& get_string() const             { return __value.get_string(); }
			slim_coordinates get_coordinates() const          { return __value.get_coordinates(); }

			// --- safe cross-type numeric getters ---

			int64_t     get_as_int64()  const { return __value.get_as_int64(); }
			uint64_t    get_as_uint64() const { return __value.get_as_uint64(); }
			double      get_as_double() const { return __value.get_as_double(); }
			long double get_as_float()  const { return __value.get_as_float(); }

			// --- optional / non-throwing getters ---

			std::optional<bool>             try_bool()        const { return __value.try_bool(); }
			std::optional<int64_t>          try_int64()       const { return __value.try_int64(); }
			std::optional<double>           try_double()      const { return __value.try_double(); }
			std::optional<std::string>      try_string()      const { return __value.try_string(); }
			std::optional<slim_coordinates> try_coordinates() const { return __value.try_coordinates(); }

			// --- value_or helpers ---

			template <typename _T>
			_T value_or(_T _fallback) const { return __value.value_or(std::move(_fallback)); }

			std::string value_or(std::string _fallback) const { return __value.value_or(std::move(_fallback)); }
			std::string value_or(const char* _fallback) const { return __value.value_or(_fallback); }
